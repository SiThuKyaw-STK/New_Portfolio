import React from 'react';
import {TbCodeCircle2} from "react-icons/tb";

const EducationOfHero = () => {
    return (
        <section id={``} className={`md:py-[1rem] md:px-[2rem] px-[1rem]`}>
            <h6 className={`text-primary font-[500] text-center`}>My Education</h6>
            <h3 className={`text-white font-bold text-center`}>
                <span className={`underline underline-offset-[5px] decoration-primary`}>Jou</span>rney
            </h3>
            {/*component*/}
            <div className="container mx-auto w-full h-full">
                <div className="relative wrap overflow-hidden lg:p-10 md:p-5 p-10 h-full">
                    <div className="border-2-2 absolute border-primary h-full border md:left-[50%] left-5"></div>
                    <div className="mb-8 md:flex justify-center items-center w-full relative">
                        <div className="order-1 bg-gray-400 rounded-lg lg:w-5/12 md:w-6/12 lg:px-6 md:px-3 py-4 md:text-end">
                            <h6 className="mb-1 font-bold text-primary leading-none">Basic Education</h6>
                            <p className="md:text-xs text-[11px] tracking-wide text-white/80 leading-none">
                                2016-2017 | Kaung Su San (Private High School)
                            </p>
                        </div>
                        <div className="z-20 flex items-center order-1 bg-primary shadow-xl w-8 h-8 rounded-full absolute md:static top-[10px] left-[-35px]">
                            <h1 className="mx-auto text-2xl text-black">
                                <TbCodeCircle2/>
                            </h1>
                        </div>
                        <div className="order-1 bg-gray-400 rounded-lg lg:w-5/12 md:w-6/12 lg:px-6 md:px-3 py-4 text-justify">
                            <p className="text-sm leading-snug tracking-wide text-white/80 text-opacity-100">Lorem Ipsum
                                is
                                simply dummy text of the printing and typesetting industry. Lorem Ipsum has been the
                                industry's standard dummy text ever since the 1500s, when an unknown printer took a
                                galley of type and scrambled it to make a type specimen book.</p>
                        </div>
                    </div>
                    <div className="mb-8 md:flex flex-row-reverse justify-center items-center w-full relative">
                        <div className="order-1 bg-gray-400 rounded-lg lg:w-5/12 md:w-6/12 lg:px-6 md:px-3 py-4">
                            <h6 className="mb-1 font-bold text-primary leading-none">Basic Education</h6>
                            <p className="md:text-xs text-[11px] tracking-wide text-white/80 leading-none">
                                2016-2017 | Kaung Su San (Private High School)
                            </p>
                        </div>
                        <div className="z-20 flex items-center order-1 bg-primary shadow-xl w-8 h-8 rounded-full absolute md:static top-[10px] left-[-35px]">
                            <h1 className="mx-auto text-2xl text-black">
                                <TbCodeCircle2/>
                            </h1>
                        </div>
                        <div className="order-1 bg-gray-400 rounded-lg lg:w-5/12 md:w-6/12 lg:px-6 md:px-3 py-4 text-justify">
                            <p className="text-sm leading-snug tracking-wide text-white/80 text-opacity-100">Lorem Ipsum
                                is
                                simply dummy text of the printing and typesetting industry. Lorem Ipsum has been the
                                industry's standard dummy text ever since the 1500s, when an unknown printer took a
                                galley of type and scrambled it to make a type specimen book.</p>
                        </div>
                    </div>
                    <div className="mb-8 md:flex justify-center items-center w-full relative">
                        <div className="order-1 bg-gray-400 rounded-lg lg:w-5/12 md:w-6/12 lg:px-6 md:px-3 py-4 md:text-end">
                            <h6 className="mb-1 font-bold text-primary leading-none">Basic Education</h6>
                            <p className="md:text-xs text-[11px] tracking-wide text-white/80 leading-none">
                                2016-2017 | Kaung Su San (Private High School)
                            </p>
                        </div>
                        <div className="z-20 flex items-center order-1 bg-primary shadow-xl w-8 h-8 rounded-full absolute md:static top-[10px] left-[-35px]">
                            <h1 className="mx-auto text-2xl text-black">
                                <TbCodeCircle2/>
                            </h1>
                        </div>
                        <div className="order-1 bg-gray-400 rounded-lg lg:w-5/12 md:w-6/12 lg:px-6 md:px-3 py-4 text-justify">
                            <p className="text-sm leading-snug tracking-wide text-white/80 text-opacity-100">Lorem Ipsum
                                is
                                simply dummy text of the printing and typesetting industry. Lorem Ipsum has been the
                                industry's standard dummy text ever since the 1500s, when an unknown printer took a
                                galley of type and scrambled it to make a type specimen book.</p>
                        </div>
                    </div>
                    <div className="mb-8 md:flex flex-row-reverse justify-center items-center w-full relative">
                        <div className="order-1 bg-gray-400 rounded-lg lg:w-5/12 md:w-6/12 lg:px-6 md:px-3 py-4 ">
                            <h6 className="mb-1 font-bold text-primary leading-none">Basic Education</h6>
                            <p className="md:text-xs text-[11px] tracking-wide text-white/80 leading-none">
                                2016-2017 | Kaung Su San (Private High School)
                            </p>
                        </div>
                        <div className="z-20 flex items-center order-1 bg-primary shadow-xl w-8 h-8 rounded-full absolute md:static top-[10px] left-[-35px]">
                            <h1 className="mx-auto text-2xl text-black">
                                <TbCodeCircle2/>
                            </h1>
                        </div>
                        <div className="order-1 bg-gray-400 rounded-lg lg:w-5/12 md:w-6/12 lg:px-6 md:px-3 py-4 text-justify">
                            <p className="text-sm leading-snug tracking-wide text-white/80 text-opacity-100">Lorem Ipsum
                                is
                                simply dummy text of the printing and typesetting industry. Lorem Ipsum has been the
                                industry's standard dummy text ever since the 1500s, when an unknown printer took a
                                galley of type and scrambled it to make a type specimen book.</p>
                        </div>
                    </div>

                </div>
            </div>
        </section>
    );
};

export default EducationOfHero;